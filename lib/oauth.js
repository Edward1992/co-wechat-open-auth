...
class OpenAuth {
  constructor(appid, appsecret, aesToken, aesKey, saveVerifyTicket, getVerifyTicket, getComponentToken, saveComponentToken) {
    ...
    this.saveComponentToken = saveComponentToken || async function (token) {
      this.store = token;
    };
  ...
  async request(url, opts = {}) {
    var options = Object.assign({timeout: 5000}, this.defaults);
    for (var key in opts) {
      if (key !== 'headers') {
  ...
  async getComponentAccessToken() {
    try {
      ...
      const token = await this.request(url, this.postJSON(params)); 
      ...
      console.log(`getComponentAccessToken: ${token}`);
      return token;
    } catch(e) {
      console.log('=== getComponentAccessTonken error ===', e);
    };  
  };
  ...
  async preRequest(method, args = [], retryed) {
    try {
      ...
      let accessToken;
      if (token) accessToken = new ComponentAccessToken(token);
        ...
        if (!retryed) {
          const retryHandle = async data => {
            ...
            if (data && data.errcode && data.errcode === 40001) {
              return (await this.preRequest(method, args, true));
            }
            return data;
          };

          const func = method.apply(this, args);
          return (await retryHandle(await func));
        } else {
  ...
  getOpenAuthorizeURL(pre_auth_code, redirect) {
    ...
    console.log(`getOpenAuthorizeURL: ${url}`)
